...
def nm_distance(label: str, key: str, auto_val: Optional[float], max_val=2000.0) -> Optional[float]:
    ...
    # two thin columns so it's 1 line
    ...
    with lcol:
        ...
        val = st.number_input(label, min_value=0.0, max_value=float(max_val), step=0.1,
                              value=st.session_state[f"{key}_val"], disabled=disabled, key=f"{key}_val")
...
with st.form("inputs"):
    ...
    with e1:
        wind_mps = st.number_input("Wind (m/s)", min_value=0.0, max_value=60.0, step=0.1,
                                   value=float(auto.get("wind_mps",0.0)))
    with e2:
        wind_deg = st.number_input("Wind dir (°)", min_value=0, max_value=359, step=1,
                                   value=int(auto.get("wind_deg",0)))
    with e3:
        slope_pct = st.number_input("Slope (%)", min_value=0.0, max_value=100.0, step=0.1,
                                    value=float(auto.get("slope_pct",0.0)))

    a1,a2,a3 = st.columns(3)
    with a1:
        approach_avg = st.number_input("Approach avg (%)", min_value=0.0, max_value=100.0, step=0.1,
                                       value=float(auto.get("approach_avg",0.0)))
    with a2:
        approach_max = st.number_input("Approach max (%)", min_value=0.0, max_value=100.0, step=0.1,
                                       value=float(auto.get("approach_max",0.0)))
    with a3:
        rr_default = "" if auto.get("rr") is None else f"{auto['rr']:.2f}"
        rr_str = st.text_input("Route indirectness (× crow-fly) — optional",
                               value=rr_default, placeholder="leave blank")
        try:
    ...
    with s2:
        ...
        land_use   = st.selectbox("Land use", ["Domestic/Urban","Industrial","Mixed","Rural/Agricultural"],
    ...
    with v2:
        stand_surface = st.selectbox("Stand surface",
            ["asphalt","concrete","block paving","gravel","grass","other"], index=0)
